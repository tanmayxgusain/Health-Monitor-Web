...
def _confidence_label(total_windows: int) -> str:
    """
    Simple user-facing confidence label based on how much data we have.
    Tune as you like.
    """
...
async def personal_anomaly(
    ...
    # windowed.index is UTC-naive timestamps; treat as UTC then convert to IST for display
    for ts_utc_naive, row, pred in zip(windowed.index, windowed.values, predictions):
        ...
        point = {
            "timestamp": ts_ist.isoformat(),  # IST ISO string for frontend
    ...
    # ---------------------------------------------------
    # NEW: Simple "top contributors" heuristic for UI chips
    # (based on volatility today; not medical diagnosis)
    # ---------------------------------------------------
    ...
    return {
        # --- KEEP THESE EXACTLY (your existing text summary) ---
        "status": "alert" if percent > 20 else "ok",
        "date": start_ist.date().isoformat(),  # IST date shown to user
        "total_records": total,
        "anomalies": anomalies,
        ...
        # --- NEW FIELDS FOR PERSONAL DASHBOARD UI ---
        "series": series,  # timeline + charts
        "anomaly_timestamps": anomaly_timestamps,  # quick markers
        "top_contributors": top_contributors,  # chips like ["Heart Rate", "SpO₂"]
        "data_confidence": _confidence_label(total),  # "low" | "medium" | "high"
    }
